...
- **`dependency-cve`** — parses changed npm v7+ `package-lock.json` files, Yarn Classic `yarn.lock` files, pnpm v5/v6-style `pnpm-lock.yaml` package entries, and exact `==` pins in `requirements*.txt`, then queries [OSV.dev](https://osv.dev) for known CVEs. Yarn Berry and unpinned or range-based Python requirements are not covered. Findings appear inline on the lockfile line with the version pin, tagged `_via OSV · <identifier>_`; the identifier preference is a CVE alias, then a GHSA alias, then the native OSV advisory ID.
