...
function buildExportFinding(
  ...
  return {
    ...
    description:
      `${what}\n\n` +
      `Knip detects this via whole-project analysis. If \`${issue.name}\` is ` +
      `intended as part of the public API, add it to your knip config's \`entry\` ` +
      `or \`ignoreExportsUsedInFile\`. To fix: remove the \`export\` keyword or ` +
      `delete the declaration if it's only used internally.`,
    confidence,
