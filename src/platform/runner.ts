...
import type { RepoFileReader, ReviewPlatform } from './types.js';
...
export async function runReviewWithPlatform(input: ReviewRunInput): Promise<OrchestratorOutput> {
  ...
  for (const secret of platform.authSecrets) {
    if (!secret) continue;
    registerSecret(secret);
    await logger.setSecret(secret);
  }
  ...
  await logger.info(
    `Starting code review for ${platform.repoLabel}#${platform.pull_number}` +
      (input.dry_run ? ' (DRY RUN)' : ''),
  );

  // Fetch PR metadata + files + diff
  const prContext = await platform.fetchPRContext();
  await logger.info(
  ...
  const providerPreference = input.provider_override ?? config.provider;
  const selection = resolveModelSelection({
    model: config.model,
    ...(providerPreference !== undefined ? { providerPreference } : {}),
    hasAnthropicKey: input.anthropic_api_key.length > 0,
    hasOpenAIKey: input.openai_api_key.length > 0,
  });
  config.model = selection.model;
  const resolvedProvider: ProviderId = selection.provider;
  ...
  if (agentScope.prContext.files.length > 0) {
    try {
      priorThreads = await platform.fetchPriorReviewThreads(prContext);
  ...
  const buildPrompt = (findings: ReadonlyArray<ScanFinding> = []): string => {
    const base = buildUserPrompt({
      ...
      repo: platform.repo,
      pull_number: platform.pull_number,
  ...
  // Inline comments still post when the human-facing summary is disabled.
  // Platform adapters retain their hidden marker so sticky review handling
  // and prior-thread loading can continue to identify Vor artifacts.
  ...
  if (config.review.sticky) {
    try {
      const dismissed = await platform.supersedePriorReviews(prContext);
      if (dismissed > 0) {
        await logger.info(`Superseded ${dismissed} prior agent review artifact(s).`);
      }
    } catch (err) {
      await logger.warn(`Failed to supersede prior reviews: ${(err as Error).message}`);
    }
  }

  // Post the review
  const posted = await platform.postReview({
  ...
  await logger.info(
    `Posted review with ${posted.comment_count} inline comment(s).` +
      (posted.review_id !== undefined ? ` GitHub review id: ${posted.review_id}.` : '') +
      (posted.post_id !== undefined ? ` Platform post id: ${posted.post_id}.` : ''),
  );

  return {
    ...(posted.review_id !== undefined ? { review_id: posted.review_id } : {}),
    ...(posted.post_id !== undefined ? { post_id: posted.post_id } : {}),
...
async function loadConfig(
  ...
  const platform = input.platform;
  // First try fetching from the PR head via the platform API (CI mode)
  try {
    ...
    await logger.debug(
      `Could not read ${input.config_path} from ${platform.id}: ${(err as Error).message}`,
    );
  }
  ...
  try {
    ...
    // Missing or unreadable local fallback config keeps the existing
    // no-config behavior: run with built-in defaults.
